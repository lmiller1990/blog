...
We can use skani's `triangle` function to calculate the distance from "the rest".
...
skani triangle -c 30 -s 70 --min-af 5 human_mastadenovirus_*/**/*.fna > skani_ani_matrix.txt

grep "human_mastadenovirus_f" skani_ani_matrix.txt | awk '{sum=0; count=0; for(i=1; i<=NF; i++) {sum+=$i; count++}} END {print "Average ANI of Adenovirus F vs Others:", sum/count}'

for group in a b c d e f g; do
    result=$(grep "human_mastadenovirus_f" skani_ani_matrix.txt | grep "human_mastadenovirus_${group}")
    
    if [[ -n "$result" ]]; then
        echo "$result" | awk '{sum=0; count=0; for(i=1; i<=NF; i++) {sum+=$i; count++}} END {if (count > 0) print "ANI between Adenovirus F and", toupper("'$group'"), ":", sum/count; else print "No valid ANI values for", toupper("'$group'")}'
    else
        echo "No valid ANI values for Adenovirus F vs ${group}"
    fi
done
